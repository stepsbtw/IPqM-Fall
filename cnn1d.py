...
class CNN1D(nn.Module):
    def __init__(self, num_features):
        ...
        self.features = nn.Sequential(
            nn.Conv1d(num_features, CONV1_CHANNELS, kernel_size=CONV1_KERNEL, padding=CONV1_KERNEL // 2),
            nn.ReLU(),
            nn.BatchNorm1d(CONV1_CHANNELS),
            nn.MaxPool1d(POOL_KERNEL),
            nn.Conv1d(CONV1_CHANNELS, CONV2_CHANNELS, kernel_size=CONV2_KERNEL, padding=CONV2_KERNEL // 2),
            nn.ReLU(),
            nn.BatchNorm1d(CONV2_CHANNELS),
            nn.MaxPool1d(POOL_KERNEL),
            nn.Conv1d(CONV2_CHANNELS, CONV3_CHANNELS, kernel_size=CONV3_KERNEL, padding=CONV3_KERNEL // 2),
            nn.ReLU(),
        ...
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(CONV3_CHANNELS, LINEAR1_UNITS),
            nn.ReLU(),
            nn.Dropout(DROPOUT),
            nn.Linear(LINEAR1_UNITS, NUM_CLASSES)
        )
